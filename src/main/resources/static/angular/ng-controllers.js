...
var loginController = function() {
	var controller = function($rootScope, $scope, $http, $location) {
		...
		// Helper function to check whether the user is already authenticated
		var authenticate = function(credentials, callback) {
			var headers = {};
			if (credentials) {
				headers["Authorization"] = "Basic " + btoa(credentials.username + ":" + credentials.password);
			}
			$http.get("/v2/security/user", { headers : headers })
		...
		// Check to see if user is already authenticated (e.g. if user refreshed browser in middle of session)
		authenticate();
	...
	return [ '$rootScope', '$scope', '$http', '$location', controller ];
}
